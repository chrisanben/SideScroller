...
class PlayerCar(pygame.sprite.Sprite):
    def __init__(self):
        ...
        self.gas = pygame.mixer.Sound("sounds/score.wav")
        self.engine = pygame.mixer.Sound("sounds/engine.wav")

        self.engine.set_volume(0.1)

        self.gas.set_volume(0.5)
        self.gas.fadeout(500)
    ...
    def update(self):
        ...
        if self.mouseAcceleration:
            #If the mouse movement is downward within a certain value
            if self.rect.centery > mousey + 2.0:
            ...
            elif self.rect.centery < mousey - 2.0:
    ...
    while keepGoing:
        ...
        if scoreboard.time > 0.33:
            if gamespeed < 60:
                ...
                scoreboard.fuel -= 5
                carspeed = 50
            scoreboard.distance = scoreboard.distance + carspeed
...
def intro(score):
    ...
    road = Road()
    logo = Logo()
    allSprites = pygame.sprite.OrderedUpdates(road, pcar, logo)

    if score != 0:
        scoreline = "Distance covered last run: %d m" % score
    else:
        scoreline = ""

    #Write/Draw instructions plus Score
    insFont = pygame.font.Font('images/Fipps.otf', 20)
    insLabels = []
    instructions = (
    "",
    scoreline,
    ...
    while keepGoing:
        ...
        for i in range(len(insLabels)):
            screen.blit(insLabels[i], (50, 100+30*i))
